...
<template>
  <div :style="statusBarStyle"
    class="fixed px-1 text-xs bottom-[-2px] x-50 flex justify-end items-center select-none border-gray-400 border border-r-white text-[var(--status-bar-color)] bg-[var(--status-bar-background)]">

    <div class="clickable px-[6px] py-[2px] max-w-48 truncate" @click="$emit('openNoteSelector')"
    ...
    <div class="line-number  px-[6px] py-[2px]">
    ...
    <div class="doc-size px-[6px] py-[2px]">{{ formatSize }}</div>
    <div @click="$emit('toggleSpellCheck')" class="clickable px-[6px] py-[2px]">
      <span v-if="isSpellChecking">Disable
        spell checking</span>
      <span v-else>Enable spell checking</span>
    </div>
    <div @click="$emit('openLanguageSelector')" class="lang clickable  px-[6px] py-[2px]" :title="changeLanguageTitle">
      {{ languageName }}
    ...
    <div v-if="supportsFormat" @click="$emit('formatCurrentBlock')" class="status-block py-0 clickable"
      ...
      <div class="icon flex items-center"><svg width="14px" height="22px" fill="none" stroke="currentColor"
    ...
    <div @click="$emit('openSettings')" class="status-block py-0 clickable" title="Settings">
      <div class="icon flex items-center"><svg width="14px" height="22px" viewBox="0 0 512 512" fill="none"
